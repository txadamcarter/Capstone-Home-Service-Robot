...
void Listener::stage_one(int stage)
{
...
int main( int argc, char** argv )
{
  ...
  loop_rate.sleep();
  ros::spinOnce(); 
  ROS_INFO("Closest: [%s], Distance: [%f]", listener.name.c_str(), listener.distance);

  while (ros::ok())
  {

    if (listener.name.c_str() == "PickupLocation" && listener.distance > 1.0)
    {

    }

    else if (listener.name.c_str() == "PickupLocation" && listener.distance <= 1.0)
    {
      // delete at pickup location
      ROS_INFO("Stage 2");
      visualization_msgs::Marker marker;
      marker.header.frame_id = "/map";
      marker.header.stamp = ros::Time::now();
    ...
    else if (listener.name.c_str() == "DropoffLocation" && listener.distance <= 1.0)
    {
      // advertise at dropoff location
      ROS_INFO("Stage 3");
