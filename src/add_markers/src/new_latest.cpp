...
int main( int argc, char** argv )
{
  ...
  while (listener.name.c_str() == "PickupLocation" && listener.distance > 1.0)
  {
...
//    ROS_INFO("Travelling");
  ...
  while (location.c_str() == "PickupLocation" && listener.distance <= 1.0) 
    {
      ROS_INFO("Stage 1");
      visualization_msgs::Marker marker;
      marker.header.frame_id = "/map";
    ...
    while (location.c_str() == "DropoffLocation" && listener.distance <= 1.0)
    {
      ROS_INFO("Stage 2");
